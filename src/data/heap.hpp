...
namespace data
{
    ...
    {
        utils :: pdelete(this->_items, this->_alloc + 1);
    }
    ...
    template <typename type> void heap <type> :: push(const type & item)
    {
        if(this->_size == this->_alloc)
        {
            ...
            this->_alloc *= 2;
            utils :: pnew <optional <type>> allocator(this->_alloc + 1);

            for(size_t i = 1; i <= this->_size; i++)
                allocator[i](old[i]);

            for(size_t i = this->_size + 1; i <= this->_alloc; i++)
                allocator[i](null);

            this->_items = allocator;
            utils :: pdelete(old, this->_size + 1);
        }

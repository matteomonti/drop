...
namespace bytewise :: visitors
{
    ...
    // iterator <vtype, mask <range <beg, end, swap>, tail...>>

    template <typename ttype> template <typename vtype, size_t beg, size_t end, bool swap, typename... tail> inline void arithmetic <ttype> :: iterator <vtype, mask <range <beg, end, swap>, tail...>> :: read(const ttype & target, vtype & visitor)
    {
        visitor.read((reinterpret_cast <const selector <beg, end, false> &> (target)).value);
        iterator <vtype, mask <tail...>> :: read(target, visitor);
    }

    template <typename ttype> template <typename vtype, size_t beg, size_t end, bool swap, typename... tail> inline void arithmetic <ttype> :: iterator <vtype, mask <range <beg, end, swap>, tail...>> :: write(ttype & target, vtype & visitor)
    {
